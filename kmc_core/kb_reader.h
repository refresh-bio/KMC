...
template <typename KMER_T, unsigned SIZE> CKmerBinReader<KMER_T, SIZE>::CKmerBinReader(CKMCParams &Params,
		CKMCQueues &Queues) {
	mm = Queues.mm;
//	dm = Queues.dm;
	bd = Queues.bd;
...
template <typename KMER_T, unsigned SIZE> void CKmerBinReader<KMER_T, SIZE>::ProcessBins() {
	...
	while((bin_id = bd->get_next_random_bin()) >= 0) {	// Get id of the next bin to read
		...
		if (!memory_bins->init(bin_id, rec_len, round_up_to_alignment(size), round_up_to_alignment(input_kmer_size),
							   round_up_to_alignment(out_buffer_size), round_up_to_alignment(kxmer_counter_size),
							   round_up_to_alignment(lut_size))) {
			tlbq->insert(bin_id);
			continue;
		}
		...
		if(size > 0) {
			...
			if(readed != size) {
				std::cout << "Error: Corrupted file: " << name << "   " << "Real size : " << readed << "   " << "Should be : "
						  << size << "\n";
				fflush(stdout);
				exit(1);
			}
...
//----------------------------------------------------------------------------------
// Constructor
template <typename KMER_T, unsigned SIZE> CWKmerBinReader<KMER_T, SIZE>::CWKmerBinReader(CKMCParams &Params,
		CKMCQueues &Queues) {
