...
void CBigKmerBinMerger<KMER_T, SIZE>::init(int32 bin_id, uint32 _size) {
	...
	for (uint32 i = 0; i < size; ++i) {
		...
		sub_bins[i]->init(file, file_size, lut_prefix_len, n_kmers, name, kmer_len,
						  sub_bin_lut_buff + i * per_sub_bin_lut_size, per_sub_bin_lut_size,
						  sub_bin_suff_buff + i * per_sub_bin_suff_size, per_sub_bin_suff_size);
...
void CBigKmerBinMerger<KMER_T, SIZE>::Process() {
	...
	while (sm_cbc->pop(bin_id)) {
		...
		bbkpq->push(bin_id, suff_buff, suff_buff_pos, raw_lut, (lut_recs - lut_offset) * sizeof(uint64), n_unique,
					n_cutoff_min, n_cutoff_max, n_total, true);
	}
