...
void RadixOMP_uint8(uint32 *SourcePtr, uint32 *DestPtr, const int64 SourceSize, unsigned rec_size,
	...
	#pragma omp parallel num_threads(n_threads)
	{
		...
		for(uint32 privatePhaseCounter = 0; privatePhaseCounter < n_phases; privatePhaseCounter++) {
			...
			for(i = data_offset; i < SourceSize_in_bytes; i = i + rec_size) {
				byteValue = *(&char_ptr_tempSource[i] + ByteIndex);
				memcpy(&char_ptr_tempDest[privateByteCounter[byteValue] * rec_size], &char_ptr_tempSource[i - data_offset],
					   rec_size);
				(privateByteCounter[byteValue])++;
			}
...
void RadixOMP_buffer(CMemoryPool *pmm_radix_buf, uint64 *Source, uint64 *Dest, const int64 SourceSize,
		...
		for(uint32 privatePhaseCounter = 0; privatePhaseCounter < n_phases; privatePhaseCounter++) {
			...
			for(i = 0; i < SourceSize; ++i) {
				...
				if(index_x == (BUFFER_WIDTH -1))
					A_memcpy ( &tempDest[privateByteCounter[byteValue] - (BUFFER_WIDTH)], &Buffer[byteValue * BUFFER_WIDTH],
							   BUFFER_WIDTH *sizeof(uint64) );
			} //end_for
			...
			for(private_i = 0; private_i < 256; private_i++) {
				...
				if(elemInBuffer != 0)
					A_memcpy ( &tempDest[privateByteCounter[private_i] - elemInBuffer],
							   &Buffer[private_i * BUFFER_WIDTH + (privateByteCounter[private_i] - elemInBuffer)%BUFFER_WIDTH],
							   (elemInBuffer)*sizeof(uint64) );
			}
