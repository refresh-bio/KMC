...
void * A_memmove(void * dest, const void * src,
				 size_t count); // Same as memcpy, allows overlap between src and dest
void * A_memset (void * dest, int c, size_t count);            // Set count bytes in dest to (char)c
size_t GetMemcpyCacheLimit(
	void);                              // Data blocks bigger than this will be copied uncached by memcpy and memmove
void   SetMemcpyCacheLimit(size_t);                            // Change limit in GetMemcpyCacheLimit
size_t GetMemsetCacheLimit(
	void);                              // Data blocks bigger than this will be stored uncached by memset
void   SetMemsetCacheLimit(size_t);                            // Change limit in GetMemsetCacheLimit
char * A_strcat (char * dest, const char *
				 src);               // Concatenate strings dest and src. Store result in dest
...
int    A_stricmp (const char *string1,
				  const char *string2);   // Compare strings. Case insensitive for A-Z only
...
size_t A_substring(char * dest, const char * source, size_t pos,
				   size_t len); // Copy a substring for source into dest
size_t A_strspn (const char * str, const char * set);          // Find span of characters that belong to set
size_t A_strcspn(const char * str,
				 const char * set);          // Find span of characters that don't belong to set
size_t strCountInSet(const char * str, const char * set);      // Count characters that belong to set
size_t strcount_UTF8(const char *
					 str);                        // Counts the number of characters in a UTF-8 encoded string
...
#else
uint32_t ReadTSC(
	void);                                     // Read microprocessor internal clock (only 32 bits supported by compiler)
#endif
...
class div_i32 {                                                // Signed 32 bit integer division
  public:
	div_i32() {
		buffer[0] = buffer[1] = 0;   // Default constructor
	}
	div_i32(int d) {
		setdivisor(d);   // Constructor with divisor
	}
	void setdivisor(int d) {
		setdivisori32(buffer, d);   // Set divisor
	}
  protected:
...
class div_u32 {                                                // Unsigned 32 bit integer division
  public:
	...
	void setdivisor(uint32_t d) {
		setdivisoru32(buffer, d);   // Set divisor
	}
  protected:
...
class div_v8i16 {                                              // vector of 8 signed integers of 16 bits
  public:
	div_v8i16() {
		buffer[0] = buffer[1] = _mm_set1_epi16(0);   // default constructor
	}
	div_v8i16(int16_t d) {
		setdivisor(d);   // constructor with divisor
	}
	void setdivisor(int16_t d) {
		setdivisorV8i16(buffer, d);   // set divisor
	}
  protected:
...
class div_v8u16 {                                              // vector of 8 unsigned integers of 16 bits
  public:
	...
	div_v8u16(uint16_t d) {
		setdivisor(d);   // constructor with divisor
	}
	void setdivisor(uint16_t d) {
		setdivisorV8u16(buffer, d);   // set divisor
	}
  protected:
	__m128i buffer[2];                                          // Internal memory
...
class div_v4i32 {                                              // vector of 4 signed integers of 32 bits
  public:
	div_v4i32() {
		buffer[0] = buffer[1] = _mm_set1_epi32(0);   // default constructor
	}
	div_v4i32(int32_t d) {
		setdivisor(d);   // constructor with divisor
	}
	void setdivisor(int32_t d) {
		setdivisorV4i32(buffer, d);   // set divisor
	}
  protected:
	__m128i buffer[2];                                          // Internal memory
...
class div_v4u32 {                                              // vector of 4 unsigned integers of 32 bits
  public:
	...
	div_v4u32(uint32_t d) {
		setdivisor(d);   // constructor with divisor
	}
	void setdivisor(uint32_t d) {
		setdivisorV4u32(buffer, d);   // set divisor
	}
  protected:
	__m128i buffer[2];                                          // Internal memory
