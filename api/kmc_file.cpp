...
// RET	: true		- if successful
//----------------------------------------------------------------------------------
bool CKMCFile::OpenASingleFile(const std::string &file_name, FILE *&file_handler, uint64 &size,
							   char marker[]) {
	char _marker[4];
	size_t result;
...
bool CKMCFile::ReadParamsFrom_prefix_file_buf(uint64 &size) {
	...
	if (kmc_version == 0x200) {
		...
		prefix_file_buf_size = (lut_area_size_in_bytes + 8) / sizeof(
								   uint64);		//reads without 4 bytes of a header_offset (and without markers)
		...
		prefix_file_buf_size = (size - 4) / sizeof(
								   uint64);		//reads without 4 bytes of a header_offset (and without markers)
		prefix_file_buf = new uint64[prefix_file_buf_size];
...
bool CKMCFile::CheckKmer(CKmerAPI &kmer, uint32 &count) {
	...
	uint32 pattern_offset = (sizeof(pattern_prefix_value)* 8) - (lut_prefix_length * 2) -
							(kmer.byte_alignment * 2);
...
bool CKMCFile::ReadNextKmer(CKmerAPI &kmer, uint32 &count) {
	...
	do {
		...
		uint64 temp_prefix = (prefix_index & prefix_mask) <<
							 off;	// shift prefix towards MSD. "& prefix_mask" necessary for kmc2 db format
...
//---------------------------------------------------------------------------------
bool CKMCFile::Info(uint32 &_kmer_length, uint32 &_mode, uint32 &_counter_size, uint32 &_lut_prefix_length,
					uint32 &_signature_len, uint32 &_min_count, uint32 &_max_count, uint64 &_total_kmers) {
	if(is_opened) {
...
bool CKMCFile::GetCountersForRead_kmc2(const std::string& read, std::vector<uint32>& counters) {
	...
	for (auto& super_kmer : super_kmers) {
		...
		for (uint32 i = std::get<0>(super_kmer) +kmer_length; i < std::get<0>(super_kmer) +std::get<1>(super_kmer);
				++i) {
...
// Auxiliary function.
//---------------------------------------------------------------------------------
bool CKMCFile::BinarySearch(int64 index_start, int64 index_stop, const CKmerAPI& kmer, uint32& counter,
							uint32 pattern_offset) {
	uchar *sufix_byte_ptr = nullptr;
	uint64 sufix = 0;
	//sufix_offset is always 56
	uint32 sufix_offset =
		56;			// the ofset of a sufix is for shifting the sufix towards MSB, to compare the sufix with a pattern
