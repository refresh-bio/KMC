...
class CSubBin {
	...
	uint32 kmer_len, lut_size, lut_buff_recs, lut_offset, cur_in_suff_buff, left_to_read, n_kmers, suff_buff_pos,
		   in_current_prefix;
	std::string name;
	FILE* file;
  ...
  public:
	...
	void init(FILE* _file, uint64 _size, uint32 _lut_prefix_len, uint32 _n_kmers, std::string _name,
			  uint32 _kmer_len, uchar* _lut_buff, uint32 _lut_buff_size, uchar* _suff_buff, uint64 _suff_buff_size);
};
...
bool CSubBin<KMER_T, SIZE>::get_min(KMER_T& kmer, uint32& count) {
	...
	if (suff_buff_pos >= cur_in_suff_buff) {
		cur_in_suff_buff = (uint32)fread(suff_buff, 1, MIN(suff_rec_len * max_in_suff_buff, left_to_read),
										 file) / suff_rec_len;
		suff_buff_pos = 0;
...
//--------------------------------------------------------------------------
template<typename KMER_T, unsigned SIZE>
void CSubBin<KMER_T, SIZE>::init(FILE* _file, uint64 _size, uint32 _lut_prefix_len, uint32 _n_kmers,
								 std::string _name, uint32 _kmer_len, uchar* _lut_buff, uint32 _lut_buff_size, uchar* _suff_buff,
								 uint64 _suff_buff_size) {
	size = _size;
	...
	rewind(file);
	read_next_lut_part();
	cur_in_suff_buff = (uint32)fread(suff_buff, 1, MIN(max_in_suff_buff * suff_rec_len, left_to_read),
									 file) / suff_rec_len;
	left_to_read -= cur_in_suff_buff * suff_rec_len;
	current_prefix = 0;
	in_current_prefix = 0;
