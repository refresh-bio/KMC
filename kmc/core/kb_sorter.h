...
template <typename KMER_T, unsigned SIZE> class CKmerBinSorter {
  ...
  public:
	...
	void set_omp_threads(uint32 _n_omp_threads) {
...
template <unsigned SIZE> class CKmerBinSorter_Impl<CKmer<SIZE>, SIZE> {
	static uint64 FindFirstSymbOccur(CKmerBinSorter<CKmer<SIZE>, SIZE> &ptr, uint64 start_pos, uint64 end_pos,
									 uint32 offset, uchar symb);
	static void InitKXMerSet(CKmerBinSorter<CKmer<SIZE>, SIZE> &ptr, uint64 start_pos, uint64 end_pos,
...
template <typename KMER_T, unsigned SIZE> CKmerBinSorter<KMER_T, SIZE>::CKmerBinSorter(CKMCParams &Params,
		CKMCQueues &Queues, int thread_no) : kxmer_set(Params.kmer_len) {
...
template <unsigned SIZE> inline void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::GetNextSymb(uchar& symb,
		uchar& byte_shift, uint64& pos, uchar* data_p) {
...
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::ExpandKmersAll(
	CKmerBinSorter<CKmer<SIZE>, SIZE>& ptr, uint64 tmp_size) {
	uint64 pos = 0;
	ptr.input_pos = 0;
	CKmer<SIZE> kmer;
...
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::ExpandKmersBoth(
	CKmerBinSorter<CKmer<SIZE>, SIZE>& ptr, uint64 tmp_size) {
	uint64 pos = 0;
	CKmer<SIZE> kmer;
	CKmer<SIZE> rev_kmer;
	...
	while (pos < tmp_size) {
		...
		//building kmer
		for (uint32 i = 0, kmer_pos = 8 * SIZE - 1, kmer_rev_pos = 0; i < kmer_bytes;
				++i, --kmer_pos, ++kmer_rev_pos) {
			kmer.set_byte(kmer_pos, data_p[pos + i]);
...
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::FromChildThread(
	CKmerBinSorter<CKmer<SIZE>, SIZE>& ptr, CKmer<SIZE>* thread_buffer, uint64 size) {
...
template<unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::ExpandKxmerBothParaller(
	CKmerBinSorter<CKmer<SIZE>, SIZE>& ptr, uint64 start_pos, uint64 end_pos) {
	uchar* _raw_buffer;
...
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::ExpandKxmersBoth(
	CKmerBinSorter<CKmer<SIZE>, SIZE>& ptr, uint64 tmp_size) {
	ptr.input_pos = 0;
...
template<unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::ExpandKxmersAll(
	CKmerBinSorter<CKmer<SIZE>, SIZE>& ptr, uint64 tmp_size) {
	ptr.input_pos = 0;
	uint64 pos = 0;
	CKmer<SIZE> kmer_mask;
...
//----------------------------------------------------------------------------------
// Uncompact the kmers
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::Expand(
	CKmerBinSorter<CKmer<SIZE>, SIZE>& ptr, uint64 tmp_size) {
...
//----
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmerQuake<SIZE>, SIZE>::Expand(
	CKmerBinSorter<CKmerQuake<SIZE>, SIZE>& ptr, uint64 tmp_size) {
	uchar *data_p = ptr.data;
	uchar *raw_buffer_input, *raw_buffer_tmp;
	ptr.memory_bins->reserve(ptr.bin_id, raw_buffer_input, CMemoryBins::mba_input_array);
	ptr.memory_bins->reserve(ptr.bin_id, raw_buffer_tmp, CMemoryBins::mba_tmp_array);
...
template <typename KMER_T, unsigned SIZE> void CKmerBinSorter<KMER_T, SIZE>::Sort() {
	...
	if (sizeof(KMER_T) == 8) {
		...
		RadixSort_uint8(_buffer_input, _buffer_tmp, sort_rec, sizeof(KMER_T), offsetof(KMER_T, data),
						SIZE*sizeof(typename KMER_T::data_t), rec_len, n_omp_threads);
...
template <unsigned SIZE> uint64 CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::FindFirstSymbOccur(
	CKmerBinSorter<CKmer<SIZE>, SIZE> &ptr, uint64 start_pos, uint64 end_pos, uint32 offset, uchar symb) {
...
//----------------------------------------------------------------------------------
template<unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::InitKXMerSet(
	CKmerBinSorter<CKmer<SIZE>, SIZE> &ptr, uint64 start_pos, uint64 end_pos, uint32 offset, uint32 depth) {
...
//----------------------------------------------------------------------------------
template<unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::PreCompactKxmers(
	CKmerBinSorter<CKmer<SIZE>, SIZE> &ptr, uint64& compacted_count) {
	compacted_count = 0;
	CKmer<SIZE> *act_kmer;
...
//----------------------------------------------------------------------------------
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::CompactKxmers(
	...
	ptr.kq->push(ptr.bin_id, out_buffer, out_pos, raw_lut, lut_size, ptr.n_unique, ptr.n_cutoff_min,
				 ptr.n_cutoff_max, ptr.n_total);
...
//----------------------------------------------------------------------------------
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::CompactKmers(
	CKmerBinSorter<CKmer<SIZE>, SIZE> &ptr) {
	uint64 i;
	uint32 kmer_symbols = ptr.kmer_len - ptr.lut_prefix_len;
	uint64 kmer_bytes = kmer_symbols / 4;
...
//----------------------------------------------------------------------------------
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmer<SIZE>, SIZE>::Compact(
	CKmerBinSorter<CKmer<SIZE>, SIZE> &ptr) {
	if (ptr.max_x)
		CompactKxmers(ptr);
	else
...
template <unsigned SIZE> void CKmerBinSorter_Impl<CKmerQuake<SIZE>, SIZE>::Compact(
	CKmerBinSorter<CKmerQuake<SIZE>, SIZE> &ptr) {
	uint64 i;
	uint32 kmer_symbols = ptr.kmer_len - ptr.lut_prefix_len;
	uint64 kmer_bytes = kmer_symbols / 4;
	uint64 lut_recs = 1 << (2 * (ptr.lut_prefix_len));
...
//----------------------------------------------------------------------------------
// Constructor
template <typename KMER_T, unsigned SIZE> CWKmerBinSorter<KMER_T, SIZE>::CWKmerBinSorter(CKMCParams &Params,
		CKMCQueues &Queues, int thread_no) {
