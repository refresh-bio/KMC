...
#include <kmc/core/kmc_typedefs.h>

#include <kmc/core/kmer.h>
#include <kmc/core/params.h>
#include <kmc/core/timer.h>
#include <kmc/core/kmc.h>
...
template<template<unsigned X> class KMER_TPL, unsigned SIZE, bool QUAKE_MODE> class CApplication {
  ...
  public:
	...
	void GetStats(double &time1, double &time2, double &time3, uint64 &_n_unique, uint64 &_n_cutoff_min,
		...
		if (is_selected) {
			kmc->GetStats(time1, time2, time3, _n_unique, _n_cutoff_min, _n_cutoff_max, _n_total, _n_reads, _tmp_size,
						  _tmp_size_strict_mem, _max_disk_usage, _n_total_super_kmers);
		} else
			app_1->GetStats(time1, time2, time3, _n_unique, _n_cutoff_min, _n_cutoff_max, _n_total, _n_reads, _tmp_size,
							_tmp_size_strict_mem, _max_disk_usage, _n_total_super_kmers);
	}
...
//		else
		...
		// Number of internal sorting threads (per single sorter)
			...
			if(tmp < MIN_SR || tmp > MAX_SR) {
				cout << "Wrong parameter: number of sotring threads per single sorter must be in range <" << MIN_SR << "," <<
					 MAX_SR << "\n";
				return false;
			} else
				Params.p_sr = tmp;
		...
		if (strncmp(argv[i], "-smso", 5) == 0) {
			...
			if (tmp < MIN_SMSO || tmp > MAX_SMSO) {
				cout << "Wrong parameter: number of sorting threads per sorter in strict memory mode must be in range <" <<
					 MIN_SMSO << "," << MAX_SMSO << "\n";
				return false;
			} else
				Params.p_smso = tmp;
		...
		if (strncmp(argv[i], "-smun", 5) == 0) {
			...
			if (tmp < MIN_SMUN || tmp > MAX_SMUN) {
				cout << "Wrong parameter: number of uncompactor threads in strict memory mode must be in range <" << MIN_SMUN
					 << "," << MAX_SMUN << "\n";
				return false;
			} else
				Params.p_smun = tmp;
		...
		if (strncmp(argv[i], "-smme", 5) == 0) {
			...
			if (tmp < MIN_SMME || tmp > MAX_SMME) {
				cout << "Wrong parameter: number of merger threads in strict memory mode must be in range <" << MIN_SMME <<
					 "," << MAX_SMME << "\n";
				return false;
			} else
				Params.p_smme = tmp;
...
int _tmain(int argc, _TCHAR* argv[]) {
	...
	if(Params.p_quake) {
		...
		app->GetStats(time1, time2, time3, n_unique, n_cutoff_min, n_cutoff_max, n_total, n_reads, tmp_size,
					  tmp_size_strict_mem, max_disk_usage, n_total_super_kmers);
		delete app;
	} else {
