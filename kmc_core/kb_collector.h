...
CKmerBinCollector::CKmerBinCollector(CKMCQueues& Queues, CKMCParams& Params, uint32 _buffer_size,
									 uint32 _bin_no) {
...
void CKmerBinCollector::PutExtendedKmer(char* seq, uint32 n) {
	...
	if (max_x) { ///for max_x = 0 k-mers (not k+x-mers) will be sorted
		if (!both_strands)
			...
			switch (max_x) {
			case 1:
				update_n_plus_x_recs<2>(seq, n);
				break;

			case 2:
				update_n_plus_x_recs<3>(seq, n);
				break;

			case 3:
				update_n_plus_x_recs<4>(seq, n);
				break;
			}
		}
	}
...
template<unsigned DIVIDE_FACTOR> void CKmerBinCollector::update_n_plus_x_recs(char* seq, uint32 n) {
	...
	rev = ((3 - seq[kmer_len - 1]) << 6) + ((3 - seq[kmer_len - 2]) << 4) + ((3 - seq[kmer_len - 3]) << 2) +
		  (3 - seq[kmer_len - 4]);
	x = 0;
