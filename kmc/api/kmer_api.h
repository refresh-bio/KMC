  ...
  protected:
	...
	inline bool from_string_impl(const RandomAccessIterator iter, uint32 len) {
		...
		if (kmer_length != len) {
			...
			if (kmer_length != 0) {
				no_of_rows = (((kmer_length + byte_alignment) % 32) ? (kmer_length + byte_alignment) / 32 + 1 :
							  (kmer_length + byte_alignment) / 32);
		...
		for (uint32 row_index = 0; row_index < no_of_rows; row_index++) {
			...
			while (i < kmer_length) {
				if ((i_in_string == 0)
						&& temp_byte_alignment) {			// check if a byte_alignment placed before a prefix is to be skipped
					temp_byte_alignment--;
					i++;
				} else {
...
// ----------------------------------------------------------------------------------
	inline CKmerAPI(uint32 length = 0) {
		if(length) {
			...
			no_of_rows = (((length + byte_alignment) % 32) ? (length + byte_alignment) / 32 + 1 :
						  (length + byte_alignment) / 32);
...
//-----------------------------------------------------------------------
	inline bool operator<(const CKmerAPI &kmer) {
		...
		for(uint32 i = 0; i < no_of_rows; i++)
			if(kmer.kmer_data[i] > kmer_data[i])
				return true;
			else if(kmer.kmer_data[i] < kmer_data[i])
				return false;
